...
public class ExportarCSV {
	...
	// Exportar a CSV (Comma-separeted value)
		// Separar por comas
		public static void exportarACSV(List<Articulos> coleccionArticulos) {
			// Con PrintWriter escribimos en el archivo "articulos.csv"
			// PrintWriter necesita en el constructor un archivo al que nosotros colocamos la extension csv
			try (PrintWriter writer = new PrintWriter(new File("articulos.csv"))) {
		        ...
		        for (Articulos articulo : coleccionArticulos) {
		            ...
		            writer.write(articulo.getId() + "," + articulo.getName() + "," +
		                    articulo.getDescription() + "," + articulo.getPrice() + "," +
		                    articulo.getStock() + "\n");
		        }

		        // Imprimimos un mensaje de confirmación indicando que la exportación se ha completado correctamente.
